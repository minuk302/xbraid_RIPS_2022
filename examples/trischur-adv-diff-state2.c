          ...
          braid_AccessStatus astatus)
{
   ...
   if (done)
   {
      ...
      vec_create(mspace, &(app->u[ii]));
      vec_copy(mspace, (u->values), (app->u[ii]));
   }
...
main(int argc, char *argv[])
{
   ...
   if (access_level > 0)
   {
      ...
      /* Print adjoint u to file */
      sprintf(filename, "%s.%03d", "trischur-adv-diff-state2.out.u", (app->myid));
      file = fopen(filename, "w");
      ...
      sprintf(filename, "%s.%03d", "trischur-adv-diff-state1.out.u0", (app->myid));
      file = fopen(filename, "w");
      for (j = 0; j < mspace; j++)
      {
         if(j!=mspace-1)
