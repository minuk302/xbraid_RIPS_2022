 ...
 ***********************************************************************EHEADER*/
 ...
 /**
 * Example:       trischur-new-adj.c
 *
 * Interface:     C
 * 
 * Requires:      only C-language support     
 *
 * Compile with:  make trischur-new-adj
 *
 * Description:  Solves a simple optimal control problem in time-parallel:
 * 
 *                 min   \int_0^1 u_1(t)^2 + u_2(t)^2 + gamma_1 c_1(t)^2  +gamma_2 c_2(t)^2 dt
 * 
 *                  s.t.  d/dt u_1(t) = u_2(t) + c_1(t)
 *                        d/dt u_2(t) = -u_2(t) + c_2(t)
 * 
...
void
apply_Vinv(double dt, double gamma_1, double gamma_2, double *v)
{
   v[0] /= 2*gamma_1*dt;
   v[1] /= 2*gamma_2*dt;
}
               ...
               braid_TriStatus status)
{
   ...
   apply_Vinv(dt, gamma_1, gamma_2, utmp);
            ...
            braid_TriStatus status)
{
    ...
    *
    * Using \tilde{C} = |( 1/dt + dt/(2*gamma_1) )           0               |
    *                   |  0                      ( 1/dt + dt/(2*gamma_2 ) ) |
    */
   rtmp = (u->values);
   if (uleft != NULL)
   {
      rtmp[0] = -rtmp[0]/(1/dt + dt/(2*gamma_1));
      rtmp[1] = -rtmp[1]/(1/dt + dt/(2*gamma_2));
   }
   else
   {
      /* At the leftmost point, use a different center coefficient approximation */
      rtmp[0] = -rtmp[0]/(1/(2*dt) + dt/(2*gamma_1));
      rtmp[1] = -rtmp[1]/(1/(2*dt) + dt/(2*gamma_2));
   }
...
main(int argc, char *argv[])
{
   ...
   gamma_1 = 0.005;            /* Relaxation parameter in the objective function */
   gamma_2 = 0.005; 
